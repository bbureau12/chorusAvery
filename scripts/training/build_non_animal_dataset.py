...
DATASET_BASE = './training/nonspecies'
...
# === DB FETCH ===
def get_all_positive_clips(non_animal_id):
    ...
    cursor.execute("""
        ...
        WHERE a.non_animal_sound_id = ?
    """, (non_animal_id,))
    results = cursor.fetchall()
    conn.close()
    return [r[0] for r in results]

def get_all_negative_clips(non_animal_id):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("""
        SELECT DISTINCT c.clip_path
        FROM Clips c
        WHERE c.id NOT IN (
            SELECT clip_id FROM ClipAnnotations WHERE non_animal_sound_id = ?
        )
    """, (non_animal_id,))
    results = cursor.fetchall()
    conn.close()
...
# === MAIN ===
def build_dataset(non_animal_id):
    base = os.path.join(DATASET_BASE, str(non_animal_id))
    paths = {
    ...
    positives = get_all_positive_clips(non_animal_id)
    print(f"✅ Found {len(positives)} positives")

    print("🔍 Getting all negatives...")
    all_negatives = get_all_negative_clips(non_animal_id)
    ...
    print("✅ Dataset build complete, properly structured, balanced, and augmented!")

if __name__ == "__main__":
    non_animal_id = int(input("Enter non_animal_sound_id to build dataset for: "))
    build_dataset(non_animal_id)
