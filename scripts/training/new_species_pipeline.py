...
def polarize_volume(audio, boost_db=12):
    """
    Boost and reduce audio volume with safety limits:
    - Always apply boost up to +15 dB (capped).
    - Only apply reduction if the clip isn't already very quiet (peak > -35 dBFS).
    Returns (boosted, reduced), where reduced can be None if skipped.
    """
    ...
    if audio.max_dBFS > -35:
        ...
        print(f"🚫 Skipping reduction: clip too quiet (peak {audio.max_dBFS:.1f} dBFS)")
        reduced = None
...
def main():
    ...
    print(f"🎧 Found {len(clips)} positive clips.")
    ...
    processed_clips = []
    for clip_file in tqdm(clips, desc="🎨 Processing clips"):
    ...
    for dataset, folder in [(train, train_dir), (val, val_dir), (test, test_dir)]:
        for f in dataset:
            ...
            os.remove(dest_wav)  # Remove WAV after generating spectrogram
...
def generate_negatives(cursor, base_dir, train_count, val_count, test_count):
    ...
    for split_name, folder, num_samples in [
        ...
        for i in range(num_samples):
            ...
            clip.export(out_path, format="wav")
            generate_mel_spectrogram(clip, out_path.replace('.wav', '.png'))
            os.remove(out_path)  # Remove WAV after generating spectrogram
