...
def main():
    ...
    slug = slug or "fine_grained_nonanimal"
    ...
    if include_animals:
        ...
        overlay_pool = [AudioSegment.from_file(os.path.join("./recordings/training_data", file)).set_channels(1).set_frame_rate(16000)
    ...
    for sound_id, sound_name in zip(selected_ids, selected_names):
        print(f"\n🔧 Processing {sound_name} as class {class_index}")
        category_slug = slugify(sound_name)
        ...
        print(f"🧾 Class '{sound_name}' ({category_slug}): {len(all_outputs)} spectrograms generated from {len(selected_clips)} starter clips")
        class_index += 1
