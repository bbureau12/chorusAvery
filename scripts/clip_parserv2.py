...
min_clip_ms = 500     # 0.5 seconds
silence_thresh_relative = -14
save_plots = False

# Boosting settings
boost_threshold_dbfs = -25.0
boost_amount_db = 15
...
def parse_chunk_start_time(base_name):
    """Extract timestamp from filename like '250511_0419_22_10_47'"""
    try:
        ...
        print(f"⚠️ Timestamp parse failed: {base_name} — {e}")
        return None, None

def generate_clip_filename(base_name, start_time):
    """Create unique clip name with timestamp"""
...
def apply_bandpass_filter(audio, lowcut=300, highcut=8000):
    """Chain high-pass and low-pass to form a bandpass filter"""
...
def slice_audio_dynamic_threshold(file_path, apply_bandpass=False, lowcut=200, highcut=12000):
    ...
    if apply_bandpass:
        print(f"🎚️ Bandpass filter: {lowcut}-{highcut} Hz")
    ...
    while current_start < len(sound):
        ...
        if window_dbfs > silence_thresh:
            ...
            if len(clip) >= min_clip_ms and clip.max_dBFS > -44:
                if clip.max_dBFS < boost_threshold_dbfs:
                    ...
                    boost_applied_db = boost_amount_db
                ...
                print(f"✅ Saved: {clip_filename} ({len(clip)} ms, boost: {boost_applied_db} dB)")

                # === Export JSON sidecar ===
                meta = {
                    "filename": clip_filename,
                    "boost_applied_db": boost_applied_db,
                    "start_time": clip_start_time.isoformat(),
                    "duration_ms": len(clip),
                    "max_dbfs": round(clip.max_dBFS, 2),
                    "avg_dbfs": round(clip.dBFS, 2)
                }
                json_filename = clip_filename.replace(".wav", ".json")
                json_path = os.path.join(output_folder, json_filename)

                try:
                    with open(json_path, "w") as jf:
                        json.dump(meta, jf, indent=2)
                    print(f"📝 JSON sidecar saved: {json_filename}")
                except Exception as e:
    ...
    if save_plots:
        try:
            ...
            for start_ms, end_ms in clip_times:
                plt.axvspan(start_ms * (len(samples) / len(sound)),
                            end_ms * (len(samples) / len(sound)),
